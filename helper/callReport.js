...
console.log("in side call report ..........................................")
const redis = config.redis

function getTotalHoldTime(redisData, now,agentId,id_user,id_department) {
    now += 72 * 1000;
  try {
    console.log("agent id --- >",agentId,id_user,id_department)
    var socket = agentActivityCallHoldSocket(agentId,id_user, 2,id_department)
    // Convert duration to seconds
    const heldDuration = Math.floor((now - redisData.holded_time) / 1000);
    return (redisData.total_hold_time || 0) + heldDuration;
  } catch(error) {
    console.log("Error updating total call duration", error);
  }
}
async function getWithRetry(key, retries = 2, delayMs = 1000) {
    for (let attempt = 1; attempt <= retries; attempt++) {
      console.log(`[Redis] Attempt ${attempt} of ${retries} - Fetching key: "${key}"`);
      const data = await redis.get(key);
      if (data !== null) {
        console.log(`[Redis] Success on attempt ${attempt} - Key "${key}" found.`);
        return data;
      } else {
        console.warn(`[Redis] Attempt ${attempt} - Key "${key}" not found.`);
      }
      if (attempt < retries) {
        console.log(`[Redis] Waiting ${delayMs}ms before next retry...`);
        await new Promise(res => setTimeout(res, delayMs));
      }
    }
    console.error(`[Redis] Failed to retrieve key "${key}" after ${retries} attempts.`);
    return null;
}
async function developmentCallEvent(evt, callData) {
    try {
        ...
        if (evt.userevent == 'callStart') {
            ...
            if(callData.app == "user" && callData.direction == "incoming"){
                callEventDBactions.firebasepushnotification(callData.appId,callData.callerNumber,callData.deptId,callData.customerId)
            }
            if (callData.callType == 'clickToCallDestination') {
                var cachedRedisStringData = await redis.get(callData.uniqueId);
                ...
                if(cachedRedisParseData){
                    ...
                    await redis.set(callData.uniqueId, JSON.stringify(cachedRedisParseData));
                }else{
                    redisData = [{...callData,
                        clicktoCallScndLeg:callData.callType,
                        clicktoCallStartTime:callData.eventTime,
                    ...
                    await redis.set(callData.uniqueId, JSON.stringify(redisData));
                }
                ...
                var cachedRedisStringData = await redis.get(callData.parentUniqueId);
                var cachedRedisParseData = JSON.parse(cachedRedisStringData)
                ...
                if (cachedRedisParseData) {
                    ...
                    if (callData.transferTo != 'smartGroup') {
                ...
                var cachedRedisParseData = [{ ...callData, event: "start" }];
        ...
        if (evt.userevent == 'smartGroup') {
            ...
            var cachedRedisStringData = await redis.get(callData.uniqueId);
            console.log('smartgroup userevent cachedRedisParseData1 ....', cachedRedisParseData)
            if (cachedRedisStringData) {
                var cachedRedisParseData = JSON.parse(cachedRedisStringData)
                console.log('smartgroup userevent cachedRedisParseData....', cachedRedisParseData)
                if (callData.eventStatus == 'start_user_check') {
                    if (cachedRedisParseData[0].callType == 'Transfer') {
                        var transferCallData = await redis.get(cachedRedisParseData[0].transferCallUniqueId);
                        transferCallData = JSON.parse(transferCallData)
                        console.log("transferCallData in transfer call data ------>", transferCallData)
                        transferCallData[0].smartGroupId = callData.smartGroupId;
                        cachedRedisParseData[0].smartGroupId = callData.smartGroupId;
                        console.log("transferCallData in transfer call data before set------>", transferCallData)
                        redis.set(cachedRedisParseData[0].transferCallUniqueId, JSON.stringify(transferCallData));
                    }
                }
                if (callData.eventStatus == 'dial_start') {
                    ...
                    if (callData.ringType != 5) {
                        callEventDBactions.firebasepushnotification(callData.userId, cachedRedisParseData[0].callerNumber)
                    }
                    // cachedRedisParseData[0].userId = callData.userId
                    if (cachedRedisParseData[0].callType == 'Transfer') {
                        var transferCallData = await redis.get(cachedRedisParseData[0].transferCallUniqueId);
                    ...
                    if (callData.stickyStatus) {
                        cachedRedisParseData[0].sticky_status = callData.stickyStatus;
                    }
                }
                if (callData.eventStatus == 'dial_answered') {
                    ...
                    let connectedData = {
                        ...
                        appId: cachedRedisParseData[0].appId,
                         callStartTime: cachedRedisParseData[0].eventTime,
                    }
                    if (callData.ringType == 5) {
                        cachedRedisParseData[0].ringType = 5
                        cachedRedisParseData[0].triedUsersIds = callData.userId
                        ...
                        callEventDBactions.update_livecall(connectedData, callData.uniqueId, 'smartgroup', callData.agentChannel,cachedRedisParseData[0].callType)
                    ...
                    // update agent id and position to smart group
                    if (callData.userPosition) {
                        let updateData = { last_answered_user: callData.userId, last_answered_user_pos: callData.userPosition }

                        callEventDBactions.update_smartgroup(updateData, callData.smartGroupId)
                        callByotApi("PATCH", `/smart-group/${callData.smartGroupId}/update-call-report/${cachedRedisParseData[0].customerId}/customer`, updateData, undefined, undefined, cachedRedisParseData[0].customerId);
                    ...
                    if (cachedRedisParseData[0].callType == 'Transfer') {
                        cachedRedisParseData[0].transferAnsweredTime = callData.eventTime,
                            callEventDBactions.popup_end({ uniqueId: callData.uniqueId, userId: cachedRedisParseData[0].transferFrom, didNumber: cachedRedisParseData[0].didNumber, callerNumber: cachedRedisParseData[0].callerNumber, type: cachedRedisParseData[0].direction })
                    }
                }
                ...
                if (getRedisData) {
                    if(getRedisData[0].isDeleted != 1){
                        redis.set(callData.uniqueId, JSON.stringify(cachedRedisParseData));
        ...
        if (evt.userevent == 'user') {
            ...
            if (callData.module == "user") {
                if (callData.eventStatus == 'dial_answered') {
                    ...
                    callEventDBactions.update_livecall(connectedData, callData.uniqueId,"user",callData.agentChannel,cachedRedisParseData[0].callType)
        ...
        if (evt.userevent == "callAnswered") {
            ...
            if (callData.type == "report") {
                if (callData.eventStatus == 'dial_answered') {
                    ...
                    if(cachedRedisParseData[0]?.callType === "callTaskSource") {
                        //autox call popup
                        callEventDBactions.emit_autox_call_popup(callData.uniqueId, callData.agentChannel, cachedRedisParseData[0]?.sourceChannel, cachedRedisParseData[0]?.userId);
                    }

                    if(cachedRedisParseData[0]?.callType === "callTaskDestination") {
                        //call answered popup for autox
                        callEventDBactions.user_call_answered(callData.uniqueId, callData.agentChannel, cachedRedisParseData[0]?.sourceChannel, cachedRedisParseData[0]?.userId);
                    }
                    ...
                    callEventDBactions.update_livecall(connectedData, callData.uniqueId,"callAnswered",callData.agentChannel,cachedRedisParseData[0].callType);
        ...
        if (evt.userevent == 'callFlowReport') {
            ...
            if (callData.type == "report") {
                let cachedRedisStringData = await redis.get(callData.uniqueId);
                console.log('cachedRedisStringData....', cachedRedisStringData)
                if (cachedRedisStringData) {
                    let cachedRedisParseData = JSON.parse(cachedRedisStringData)
                    ...
                    if(callData.module == 'gotoCallflow'){
                        let moduleId = callData.moduleId;
                        await callEventDBactions.getCallflowName(moduleId,cachedRedisParseData[0]);

                    }
                    // if (callData.module == 'apiResponseRouting') {
                    //     let moduleId = callData.moduleId
                    //     const uniqueId = callData.uniqueId;
                    //     const { didNumber, userId, appId: callflowId, customerId, callerNumber  } = cachedRedisParseData[0] || {}
                        
                    //     const response = await ivrController.call_flow_send_api_response({ didNumber, userId, moduleId, uniqueId, callflowId, id_user: 271, customerMobile: callerNumber });

                    //     console.log("📞 Api Routing Response", {
                    //         userId,
                    //         didNumber,
                    //         moduleId,
                    //         uniqueId,
                    ...
                    console.log('cachedRedisParseData length....', cachedRedisParseData.length)
                    let getRedisData = await redis.get(callData.uniqueId);
                    console.log('getRedisData....', getRedisData)
                    if (getRedisData) {
                        if(getRedisData[0].isDeleted != 1){
        ...
        if (evt.userevent == 'callcenter') {
            ...
            if (callData.event == 'end') {
                ...
                if (cachedRedisParseData[0].callTransferedTo == 'smargroup') {
                    ...
                    callEventDBactions.insert_smartgroup_report(smartgroupData.result)
                }
            }
            campaignHelper.campaignCallEvent(callData)
        }
        if (evt.userevent == 'callEnd') {
            ...
            calleventlog(callData)
            // 'callEnd' event may be triggered before other related events have stored data, so retry until the data becomes available
            let cachedRedisStringData = await getWithRetry(callData.uniqueId);
            // let cachedRedisStringData = await redis.get(callData.uniqueId);
            let cachedRedisParseData = JSON.parse(cachedRedisStringData)
            ...
            if(cachedRedisParseData.length != 0){
                cachedRedisParseData[0].isDeleted = 1
            }
            await redis.set(callData.uniqueId, JSON.stringify(cachedRedisParseData));
            
            //get and save total hold duration
            if (cachedRedisParseData[0].holded_time) {
                const total_hold_time = getTotalHoldTime(cachedRedisParseData[0], now,cachedRedisParseData[0].userId,cachedRedisParseData[0].customerId,cachedRedisParseData[0].deptId);
                cachedRedisParseData[0].total_hold_time = total_hold_time;
            }
            ...
            if (cachedRedisParseData[0].callType == 'Transfer') {
                ...
                if (cachedRedisParseData[0].initial_application == 'callflow') {
                    var dtmfSequence = callEventDBactions.get_dtmf_sequence(cachedRedisParseData)
                    console.log("dtmfSequence in transfer---->", dtmfSequence);
                    if (dtmfSequence) {
                        cachedRedisParseData[0].dtmf_sequence = dtmfSequence
                    }
                }
            ...
            if (cachedRedisParseData[0].app == 'smartgroup') {
                ...
                callEventDBactions.insert_smartgroup_report(cachedRedisParseData)
            }
            if (cachedRedisParseData[0].app == 'callflow') {
                var dtmfSequence = callEventDBactions.get_dtmf_sequence(cachedRedisParseData)
                console.log("dtmfSequence ---->", dtmfSequence);
            ...
            if (cachedRedisParseData[0].callType == 'callTaskDestination') {
                ...
                callEventDBactions.add_autox_calllog(cachedRedisParseData)
                console.log("eventStatus ----------------------------->", cachedRedisParseData[0].eventStatus)
                if (cachedRedisParseData[0].eventStatus == 'dial_answered') {
                    console.log("callType is callTask ----------------------------->", cachedRedisParseData[0].callType)
                    console.log("callProcessId is callTask ----------------------------->", cachedRedisParseData[0].callProcessId)
                    console.log("callStatus is callTask ----------------------------->", values.callStatus)
                    if (values.callStatus == 'ANSWER') {
                        var call_status = 2
                    } else {
                        var call_status = 1
                    }
                    console.log("call_status is callTask ----------------------------->", call_status)
                    callEventDBactions.update_call_task_contacts(cachedRedisParseData[0].callProcessId, values.callStatus, call_status, cachedRedisParseData[0].answeredTime)
                } else {
                    callEventDBactions.update_call_task_contacts(cachedRedisParseData[0].callProcessId)

                }
            }
            if (cachedRedisParseData[0].callType == 'mobileApp_callTask') {
                console.log("eventStatus ----------------------------->", cachedRedisParseData[0].eventStatus)
                if (cachedRedisParseData[0].eventStatus == 'dial_answered') {
                    console.log("callType is mobileApp_callTask ----------------------------->", cachedRedisParseData[0].callType)
                    console.log("callProcessId is mobileApp_callTask ----------------------------->", cachedRedisParseData[0].callProcessId)
                    console.log("callStatus is mobileApp_callTask ----------------------------->", values.callStatus)
                    if (values.callStatus == 'ANSWER') {
                        var call_status = 2
                    } else {
                        var call_status = 1
            ...
            if (cachedRedisParseData[0].callType == 'clickToCallSource' || cachedRedisParseData[0].callType == 'clickToCallDestination') {
                // if (cachedRedisParseData[0].clicktoCallAnswertTime == undefined && callData.callType == 'clickToCallSource') {
                ...
                //         callEventDBactions.add_user_call_reports(cachedRedisParseData)
                //     }
                //     callEventDBactions.insert_callreport(cachedRedisParseData[0], values)
                //     redis.del(callData.uniqueId);
                // } else 
                ...
                if (cachedRedisParseData[0].clicktoCallAnswertTime == undefined && callData.callType == 'clickToCallSource') {
                    callEventDBactions.delete_livecall(cachedRedisParseData[0].uniqueId)
                    // redis.del(callData.uniqueId);
                    setTimeout(() => {
                        redis.del(callData.uniqueId);
                    }, 2000);
                ...
                console.log('callData callType ------------->', callData.callType)
                if (cachedRedisParseData[0].connectSmartgroupCallAnswertTime == undefined && callData.callType == 'connectSmartgroupSource') {
                    callEventDBactions.delete_livecall(cachedRedisParseData[0].uniqueId)
                    // redis.del(callData.uniqueId);
                    setTimeout(() => {
                        redis.del(callData.uniqueId);
                ...
                if (callData.callType == 'Transfer') {
                    ...
                    if(cachedRedisParseData[0].transferTo == "smartGroup"){
                        cachedRedisParseData[0].final_app_id = cachedRedisParseData[0].smartGroupId;
                    }else if(cachedRedisParseData[0].transferTo == "user"){
                        cachedRedisParseData[0].final_app_id = cachedRedisParseData[0].userId;
                    }else{
                         cachedRedisParseData[0].final_app_id = cachedRedisParseData[0].appId;
                    }
                    let dalldata = await redis.get(cachedRedisParseData[0].parentUniqueId);
                    ...
                    if(dalldataParse){
                        ...
                        cachedRedisParseData[0].app = dalldataParse[0].app;
                        cachedRedisParseData[0].appId = dalldataParse[0].appId;
                        cachedRedisParseData[0].cr_file = dalldataParse[0].crFile
                        values.cr_file = dalldataParse[0].crFile;
                        values.callStatus = dalldataParse[0].callStatus;
                        await redis.set(cachedRedisParseData[0].uniqueId, JSON.stringify(cachedRedisParseData));
                        // redis.del(cachedRedisParseData[0].parentUniqueId);
                        setTimeout(() => {
                            redis.del(cachedRedisParseData[0].parentUniqueId);
                        }, 2000);
                    }
                ...
                if (cachedRedisParseData[0].isTransfer != 1) {
                    callEventDBactions.insert_callreport(cachedRedisParseData[0], values)
                    // redis.del(callData.uniqueId);
                    // redis.del(callData.uniqueId, (err, result) => {
                    //     if (err) {
