...
async function insert_template(req, res, next) {
  try {
    ...
    var isSubAdmin = req.token.isSubAdmin;
    var isDept = req.token.isDept;
    var Data_type = req.body.type;
    var templates = req.body.templates;
    var agent = req.body.agent;
    var smartgroup = req.body.smartgroup;
    const day_count = req.body.day_count
    if (agent != undefined) {
      var agentId = agent.map((entry) => entry.split("_")[0]);
    }
    if (smartgroup != undefined) {
      var totalSql = `SELECT smart_group_agents.user_id FROM smart_group_agents  where smart_groupId = '${smartgroup}'  `;
      var [total_agents_via_smartgroup] = await getConnection.query(totalSql);
      if (total_agents_via_smartgroup.length != 0) {
        var agentId = total_agents_via_smartgroup.map((item) => item.user_id);
      }
    }
    var status = req.body.status;
    var status_id = Number(req.body.status_id);
    var agentCheck = req.body.agentCheck;
    var statusCheck = req.body.statusCheck;
    var order = { uniqNo: -1 };
    if (isAdmin == 1) {
      var id_user = req.token.id_user;
      if (req.body.deptId != undefined) {
        var id_department = req.body.deptId;
      } else {
        var id_department = 0;
      }
    } else if (isSubAdmin == 1) {
    ...
    if (process.env.PROJECT_NAME == "innovation") {
      var decreasedTime = currentDate.getTime() - 1.3 * 60 * 60 * 1000;
      var createdAtDate = new Date(decreasedTime);
    } else {
      var createdAtDate = new Date();
    }

    var dataArr = templates.map((obj) => Object.keys(obj));
    var result = await templatefieldModel.find(
      { template_id: new ObjectId(req.body.templateId) },
      { field_name: 1, sub: 1, _id: 0 }
    );
    var resultArr = ["phnNo"];
    if (statusCheck == false) {
      resultArr.push("status");
    }
    if (agentCheck == false) {
      resultArr.push("User Employee ID");
    }
    result.map((data) => {
      resultArr.push(data.field_name);
      if (data._doc.sub != undefined && data._doc.sub.length != 0) {
        data.sub.map((sub) => {
          resultArr.push(sub.field_name);
        });
      }
    });

   const keysToIgnore = ['name', 'status', 'email'];

   const cleanData = dataArr[0].filter(key => !keysToIgnore.includes(key)).sort();
   const cleanResult = resultArr.filter(key => !keysToIgnore.includes(key)).sort();

    const areEqual = JSON.stringify(cleanData) === JSON.stringify(cleanResult);
    if (areEqual) {
      var agentArr = [];
      var phoneNoArr = [];
      var statusArr = [];
      templates.map(async (agentData) => {
        agentArr.push(agentData["User Employee ID"]);
        statusArr.push(agentData.status);
      });
      var agentIdArr = [];
      if (agentArr.length != 0) {
        var agentSql = `SELECT id,employee_id FROM user WHERE employee_id IN('${agentArr.join(
          "','"
        )}')`;
        var [agentRes] = await getConnection.query(agentSql);
        if (agentRes.length != 0) {
          agentRes.map(async (agentId) => {
            agentIdArr.push(agentId.id);
          });
        }
      }
      statusArr = statusArr.filter((item) => item !== undefined);
      if (statusArr.length != 0) {
        console.log(statusArr);
        if (req.body.type == 1) {
          var statusSql = `SELECT id,name FROM customer_status where name IN('${statusArr.join(
            "','"
          )}')`;
          var [status] = await getConnection.query(statusSql);
          var userRoleSql = `SELECT user_id,role FROM user_role WHERE role = 2`;
          var [userRoleRes] = await getConnection.query(userRoleSql);
        }
        if (req.body.type == 2) {
          var statusSql = `SELECT id,name FROM lead_status where name IN('${statusArr.join(
            "','"
          )}')`;
          var [status] = await getConnection.query(statusSql);
          var userRoleSql = `SELECT user_id,role FROM user_role WHERE role in(2,3)`;
          var [userRoleRes] = await getConnection.query(userRoleSql);
        }
        if (req.body.type == 3) {
          var statusSql = `SELECT id,name FROM ticket_status where name IN('${statusArr.join(
            "','"
          )}')`;
          var [status] = await getConnection.query(statusSql);
          var userRoleSql = `SELECT user_id,role FROM user_role WHERE role in(2,4)`;
          var [userRoleRes] = await getConnection.query(userRoleSql);
        }
      }
      if (userRoleRes != undefined) {
        if (agentCheck == false) {
          var isUserIdIncluded = userRoleRes.some((role) =>
            agentIdArr.includes(role.user_id)
          );
          // if (isUserIdIncluded == false) {
          //   res.locals.result = "another crm";
          //   next();
          // }
        }
      }
      if (Data_type == 1) {
        const result1 = await agentModel
          .findOne({ id_user: id_user })
          .select({ uniqNo: 1, _id: 0 })
          .sort(order);
        if (result1 != null) {
          var uniqNo = Number(result1.uniqNo) + 1;
        } else {
          var uniqNo = 1000 + 1;
        }
      }
      if (Data_type == 2) {
        const result1 = await leadModel
          .findOne({ id_user: id_user })
          .select({ uniqNo: 1, _id: 0 })
          .sort(order);
        if (result1 != null) {
      ...
      if (Data_type == 3) {
        const result1 = await ticketsModel
          .findOne({ id_user: id_user })
          .select({ uniqNo: 1, _id: 0 })
          .sort(order);
        if (result1 != null) {
      ...
      var map_result = Promise.all(
        templates.map(async (data, index) => {
          data.id_user = id_user;
          data.deptId = id_department;
          data.id_template = req.body.templateId;
          ...
          if (data.createdAt == undefined) {
            ...
            var dateSplit = data.createdAt.split(" ");
          ...
          if (agentCheck == true) {
            if (agentId != undefined && agentId.length != 0) {
              data.user_id = agentId[index % agentId.length];
            }
          } else {
            if (agentRes.length != 0) {
              agentRes.map(async (agentId) => {
                if (agentId.employee_id == data["User Employee ID"]) {
                  data.user_id = agentId.id;
                  data.assignedTo = agentId.id;
                }
              });
            }
          }
          delete data["User Employee ID"];
          if (statusCheck == true) {
            if (status.length != 0) {
              data.status = status;
            }
          ...
          if (data.phnNo == "") {
            phoneNoArr.push(data);
          }
          var updateUniqno = uniqNo++;
          data.uniqNo = updateUniqno;
          return data;
        })
      );
      var output = await map_result;
      ...
      output = output.filter(
        (item) => item.phnNo !== "" && item.createdAt !== "Invalid date"
      );
      if (Data_type == 1) {
        var insertManyresult = await agentModel.insertMany(output);
      }
      if (Data_type == 2) {
        var insertManyresult = await leadModel.insertMany(output);
      }
      if (Data_type == 3) {
        var insertManyresult = await ticketsModel.insertMany(output);
      }
      let obj = {add_to_followup : true ,id_template:  output[0].id_template ,id_user: id_user,id_department: id_department?id_department:0,day_count: day_count,csvData: insertManyresult,Data_type}
      const defaultResult = await crmCallTaskContacts(obj,req.token,3)
      var succes_result = [
        { "phone number is empty this is not inserted": phoneNoArr },
        { result: insertManyresult },
      ];
      res.locals.result = succes_result;
      next();
    } else {
      res.locals.result = "not matched fields";
      next();
    }
  } catch (err) {
    console.log(err);
